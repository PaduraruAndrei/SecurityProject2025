...
class KeyboardClient:
  def __init__(self, host_addr, auto_ack=False):
    ...
    def send_modifier_only(self, mods):
        mod_byte = 0
        for mod in mods:
            mod_byte |= mod.value  # Combine modifier bits
        report = bytes([mod_byte, 0] + [0]*6)  # Modifier + reserved + 6 keys
  ...
  def send_keypress(self, *args, delay=0.1):
    # By default we wait 0.1 seconds between key presses
    # To make the attack more efficient, we changed -
    # this to 0.01 seconds. 
